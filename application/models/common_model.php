...
class Common_model extends CI_Model{
	...
	public function get_contact_purview() { 
		...
		$right = $id;//add by michen 20171231 for right
		$id  = strlen($id)>0 ? $id : 0;
		$where = $data['roleid']>0  && strlen($right)>0 ? ' and id in('.$id.')' : '';//add by michen 20171231 for right
	    return $where; 
	}
	...
	public function insert_option($key,$val) {//add for more
	    $this->jxcsys = $this->session->userdata('jxcsys');//session3
	    $this->canOperate = ($this->jxcsys['roleid'] != 0 && $this->jxcsys['orgId'] && $this->jxcsys['orgLevel']==1);
		if(!$this->canOperate) return false;//die('只有总店长才可以修改系统配置哦！不然会乱套的哦！');
		if (!$this->get_option($key,false)) {
	...
	public function update_option($key,$val) {//add for more
	    $this->jxcsys = $this->session->userdata('jxcsys');
		$data['option_value'] = serialize($val);
		return $this->mysql_model->update('options',$data,array('option_name'=>$key,'topId'=>$this->jxcsys['topId']));
	}
 
	 
	public function get_option($key,$pub=true) {//add for more
	    $this->jxcsys = $this->session->userdata('jxcsys');
	    $option_value = $this->mysql_model->get_row('options',array('option_name'=>$key,'topId'=>$this->jxcsys['topId']),'option_value');
		$pub&&($option_value||$option_value = $this->mysql_model->get_row('options',array('option_name'=>$key,'topId'=>0),'option_value')); 
