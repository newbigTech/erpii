...
class Right extends CI_Controller {
	...
	public function queryAllUser() {
	    ...
	    $where = '(1=1)';
	    if($this->jxcsys['roleid'] != 0){
	        if(!$this->jxcsys['orgId'])
	            $where = '(1=2)';
	        else
	            $where = $this->jxcsys['orgWhere'];//orgMidWhere
	    }
		$list = $this->data_model->get_admin($where,2,'roleid');
		//add for more
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['orgId']        = $row['orgId'];
			$v[$arr]['orgName']        = $row['orgName'];
		}
		$json['status'] = 200;
		$json['msg']    = 'success'; 
