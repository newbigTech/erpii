...
//add for mddb
class InvMddb extends CI_Controller {
	...
	public function index() {
		...
		switch ($action) {
			case 'initMddb':
			    ...
			    $this->load->view('scm/invMddb/initMddb');	
				break;  
			case 'editMddb':
			...
			case 'initMddbList':
			    $this->common_model->checkpurview(144);
			    $this->load->view('scm/invMddb/initMddbList');
				break; 
			default: 
			    $this->common_model->checkpurview(144); 
			    $this->mddbList();	
	...
	public function mddbList(){
		...
		$where = 'a.isDelete=0 and a.transType=104091'; 
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['inMdName']           = $inMdName[$row['id']];
		}
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['page']      = $page;
	...
	public function exportInvMddb(){
		...
		$this->load->view('scm/invMddb/exportInvMddb',$data);	
	}
	...
	public function add(){
		...
		if (strlen($data)>0) {
			...
			$this->invoice_info($iid,$data,$info);
	...
	public function update() {
		...
		$data = $this->mysql_model->get_rows('invoice',array('id'=>$id,'transType'=>104091,'isDelete'=>0));
		if (count($data)>0) {
		    ...
		    foreach ($postData['entries'] as $arr=>$row) {
				...
				$v[$arr]['inMdId'] = $row['inMdId'];
				$v[$arr]['inMdName'] = $row['inMdName'];
			}
			$json['status']                 = 200;
			$json['msg']                    = 'success'; 
	...
	public function updateInvMddb(){
		...
		if (strlen($data)>0) {
			...
			$this->invoice_info($data['id'],$data,$info);
			if ($this->db->trans_status() === FALSE) {
			    $this->db->trans_rollback();
				str_alert(-1,'SQL错误'); 
			} else {
    ...
    public function toPdf() {
		...
		$data = $this->data_model->get_invoice('a.id='.$id.' and a.transType=104091',1);  
		if (count($data)>0) { 
			$data['num']    = 53;
			...
			ob_start();
			$this->load->view('scm/invMddb/toPdf',$data);
			...
			try {
				...
				$html2pdf->Output('invMddb_'.date('ymdHis').'.pdf');
    ...
    public function delete() {
	    ...
	    $id   = intval($this->input->get('id',TRUE));
		$data = $this->mysql_model->get_rows('invoice',array('id'=>$id,'transType'=>104091));  
		if (count($data)>0) {
	...
	private function validform($data) {
		...
		$data['billType']        = 'TRANSFERMDDB';
		$data['transType']       = 104091;
		$data['transTypeName']   = '门店调拨单';
		...
		if ($data['id']>0) {
		    $invoice = $this->mysql_model->get_rows('invoice',array('id'=>$data['id'],'transType'=>104091,'isDelete'=>0));  
			count($invoice)<1 && str_alert(-1,'单据不存在、或者已删除');
			$data['checked'] = $invoice['checked'];	
			$data['billNo']  = $invoice['billNo'];	
		} else {
		    $data['billNo']  = str_no('MDDB');    
		}
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			$this->jxcsys = $this->session->userdata('jxcsys');
			$outMd = $this->jxcsys['lowId'];
			$inMd = intval($row['inMdId']);
			intval($outMd) == intval($inMd) && str_alert(-1,'调出门店不能与调入门店相同！');
	...
	//组装数据
	private function invoice_info($iid,$data,$info) {
