...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		$list = $this->data_model->get_goods($where.' order by a.id desc limit '.$rows*($page-1).','.$rows);//add for more
		//$order = ' order by a.id desc limit '.$rows*($page-1).','.$rows;//add for more
		//$list = $this->data_model->get_goods($where,$order);//add for more
		...
		$json['msg']    = 'success'; 
		$json['data']['page']      = $page;                                                            
		$json['data']['records']   = $this->data_model->get_goods($where,3);   //add for more ,$order
	...
	public function listBySelected() { 
		...
		$list = $this->data_model->get_invoice_info('a.isDelete=0 and transType='.$arr[$type].' and a.invId='.$id.' and a.buId='.$contactid.' order by a.id limit 0,3',2);//add for more
		foreach ($list as $arr=>$row) {
		    $v[$arr]['advanceDays']  = 0;
