...
class Dataright extends CI_Controller {
	...
	public function query() {
		...
		if (count($data)>0) {
			switch ($type) {
				case 1:
					...
					$list = $this->mysql_model->get_results('storage','(isDelete=0) '.$where,'id desc',$rows*($page-1),$rows); 
				...
				case 2:
					...
					$list = $this->mysql_model->get_results('contact','(isDelete=0) and type=-10 '.$where,'id desc',$rows*($page-1),$rows); 
					foreach ($list as $arr=>$row) {
						$v[$arr]['FITEMID']     = intval($row['id']);
						$v[$arr]['FNAME']       = $row['name'];
				...
				case 8:
					...
					$list = $this->mysql_model->get_results('admin','(1=1) '.$where,'uid desc',$rows*($page-1),$rows);  
					foreach ($list as $arr=>$row) {
		...
		$data['msg']    = 'success'; 
		$data['data']['rows']       = $v;
		$data['data']['total']      = ceil(count($v)/$rows);
