...
class Report extends CI_Controller {
	...
	public function deliverSummary() {
		...
		foreach ($list as $arr=>$row) {
			...
			$qty_7  += abs($row['qty21']);//add for 门店成本统计
			for ($i=8;$i<13;$i++) {
			...
			$qty_13  += abs($row['qty22']);//add for 门店成本统计
			$v[$arr]['invNo']         = $row['invNumber'];  
			$v[$arr]['invName']       = $row['invName'];   
			$v[$arr]['spec']          = $row['invSpec']; 
			$v[$arr]['unit']          = $row['mainUnit']; 
			$v[$arr]['location']      = $row['locationName']; 
			$v[$arr]['locationNo']    = $row['locationNo'];
			$v[$arr]['qty_21']   = round(abs($row['qty21']),$this->systems['qtyPlaces']);//add for 门店成本统计
			$v[$arr]['qty_22']   = round(abs($row['qty22']),$this->systems['qtyPlaces']);//add for 门店成本统计
			for ($i=0; $i<15; $i++) {
			    if ($i==0) {
		...
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['stoNames']  = array("期初","门店入库","调拨入库","普通采购","销售退回","盘盈","其他入库","成本调整","入库合计","门店出库","调拨出库","采购退回","普通销售","盘亏","其他出库","出库合计","结存");
		$json['data']['colNames']  = array("商品编号","商品名称","规格型号","单位","仓库","数量","成本","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","数量","成本");
		$json['data']['colIndex']  = array("invNo","invName","spec","unit","locationNo","qty_0","cost_0","qty_21","qty_1","qty_2","qty_3","qty_4","qty_5","qty_6","qty_7","qty_22","qty_8","qty_9","qty_10","qty_11","qty_12","qty_13","qty_14","cost_14");//add for 门店成本统计
		$json['data']['page']      = $page;
		$json['data']['records']   = 1;   
