...
<script type="text/javascript">
//add for more
$("#curLowOrg").text("<?php echo $orgName;?>");
//add for more
function showOrg(){
		var d = ['<form id="manage-form" action="">', 
			 		...
			 		'<div class="label-wrap"><label for="ParentCategory">选择门店:</label></div>', 
		...
		dialog = $.dialog({
			...
			ok: function() {
				...
				var data = {};
				data.orgId = orgId;
				Public.ajaxPost("../basedata/org/changeLowOrg", data, function(a) {
					200 == a.status ? (parent.parent.Public.tips({
						content: "操作成功！"
					}), $("#curLowOrg").text(orgName)) : parent.parent.Public.tips({
						type: 1,
						content: "操作失败！" + a.msg
					})
				})
