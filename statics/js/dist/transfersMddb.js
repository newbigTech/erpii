var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var b = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				...
				f = "",
				g = "";
			false ? (e = '<a class="ui-btn" id="audit">审核</a>', f = '<a class="ui-btn" id="reAudit">反审核</a>') : this.$_checkName.parent().hide();
		...
		loadGrid: function(a) {
			...
			function mm(a, b, c) {
				if ("get" === b) {
					if ("" !== $(".inStorageMD").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr");
					return d.removeData("inStorageMD"), ""
				}
				"set" === b && $("input", a).val(c)
			}
			function nn() {
				$("#initCombo").append($(".inStorageMD").val(""))
			}
			function o(a, b) {
				var c = $(".unitAuto")[0];
				return c
			...
			y.mod_PageConfig.gridReg(C, D), D = y.mod_PageConfig.conf.grids[C].colModel, $("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					"inMdName" === b && ($("#" + d + "_inMdName", "#grid").val(c), THISPAGE.inStorageMDCombo.selectByText(c));
					if ("prodDate" === b) {
						var g = $("#" + a).data("goodsInfo");
						if (!g) return $("#grid").jqGrid("restoreCell", d, e), curCol = e + 1, void $("#grid").jqGrid("nextCell", d, e + 1);
		...
		initCombo: function() {
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
			...
			}).getCombo(), this.inStorageMDCombo = $(".inStorageMD").combo({
				data: function() {
					return "../basedata/org/getMd"
				},
				text: "name",
				value: "id",
				defaultSelected: 0,
				...
				callback: {
					onChange: function(a) {
						var b = this.input.parents("tr"),
							c = b.data("inStorageMD");
						c || (c = {}), a && (c.id = a.id, c.name = a.name), b.data("inStorageMD", c)
					}
				}
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invMddb/add?action=add", {
				...
				if (b.preventDefault(), Business.verifyRight("TF_UPDATE")) {
					...
					c && Public.ajaxPost("../scm/invMddb/updateInvMddb?action=updateInvMddb", {
				...
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invMddb/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
				...
				a.preventDefault(), Business.verifyRight("TF_ADD") && parent.tab.overrideSelectedTabItem({
					...
					text: "调拨单",
					url: "../scm/invMddb?action=initMddb"
				})
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), a.idPostion && (loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("/scm/invMddb.do?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), a.idPostion && (loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("/scm/invMddb.do?action=update", {
					id: a.transfersListIds[a.idPostion]
				}, function(a) {
				...
				if (b.preventDefault(), Business.verifyRight("TF_CHECK")) {
					...
					d && c.ajaxPost("../scm/invMddb/checkInvTf?action=checkInvTf", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("TF_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invMddb/rsBatchCheckInvTf?action=rsBatchCheckInvTf", {
						id: originalData.id
					}, function(b) {
		...
		_getEntriesData: function() {
			for (var a = [], b = $("#grid").jqGrid("getDataIDs"), c = 0, d = b.length; d > c; c++) {
				...
				if ("" !== g.goods) {
					...
					if (!mm || !mm.id) return parent.Public.tips({
						type: 2,
						content: "请选择调入门店！"
					}), $("#grid").jqGrid("editCellByColName", f, "inMdName"), !1;
	...
	originalData;
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/invMddb/update?action=update", {
		id: urlParam.id
	}, function(a) {
