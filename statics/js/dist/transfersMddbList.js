		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "门店调拨单",
					url: "../scm/invMddb?action=editMddb&id=" + b
				...
				if (a.preventDefault(), Business.verifyRight("TF_DELETE")) {
					...
					$.dialog.confirm("您确定要删除该调拨记录吗？", function() {
						Public.ajaxGet("../scm/invMddb/delete?action=delete", {
							id: b
						}, function(a) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join(),
					d = c ? "&id=" + c : "",
					e = "../scm/invMddb/toPdf?action=toPdf" + d;
				$(this).attr("href", e)
				...
				var f = "../scm/invMddb/exportInvMddb?action=exportInvMddb" + d;
				$(this).attr("href", f)
			}), billRequiredCheck) {
				...
				$(".wrapper").on("click", "#audit", function(a) {
					if (a.preventDefault(), Business.verifyRight("TF_CHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invMddb/batchCheckInvMddb?action=batchCheckInvMddb", {
							id: c
						}, function(a) {
					...
					if (Business.verifyRight("TF_UNCHECK")) {
						...
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invMddb/rsBatchCheckInvMddb?action=rsBatchCheckInvMddb", {
							id: c
						}, function(a) {
							200 === a.status ? parent.Public.tips({
								content: a.msg
